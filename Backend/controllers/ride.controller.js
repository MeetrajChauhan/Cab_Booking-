...
module.exports.confirmRide = async (req, res) => {
  ...
  try {
    ...
    // Calculate simple ETA from captain's current location to pickup
    let etaToPickup = null;
    try {
      if (
        ride?.captain?.location &&
        Array.isArray(ride.captain.location.coordinates) &&
        ride.captain.location.coordinates.length === 2
      ) {
        const [captainLng, captainLat] = ride.captain.location.coordinates;
        ...
        const etaMinutes = Math.max(1, Math.round(distanceKm * 2)); // 2 min per km
      ...
      console.error("[Ride] Error calculating ETA to pickup:", e.message);
    }

    const ridePayload = ride.toObject ? ride.toObject() : ride;
    if (etaToPickup) {
      ridePayload.etaToPickup = etaToPickup;
    }
    ...
    return res.status(200).json(ridePayload);
  } catch (err) {
    return res.status(500).json({ message: err.message });
  }
};
