...
module.exports.authUser = async (req, res, next) => {
  ...
  try {
    ...
    req.user = {
      ...
      role: user.role || "user", // Include role from JWT
    };
    req.userType = "user";
...
module.exports.authCaptain = async (req, res, next) => {
  ...
  try {
    ...
    if (captain.accountStatus && captain.accountStatus !== "active") {
      return res.status(403).json({ message: "Captain account is not active" });
    }
    req.captain = {
      _id: captain._id,
      fullname: {
      ...
      accountStatus: captain.accountStatus || "active",
